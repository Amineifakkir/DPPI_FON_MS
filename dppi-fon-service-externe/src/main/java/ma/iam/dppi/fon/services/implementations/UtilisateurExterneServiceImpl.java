...
import ma.iam.dppi.fon.services.IUtilisateurExterneService;
...
public class UtilisateurExterneServiceImpl implements IUtilisateurExterneService {
	...
	public UtilisateurDto updateUtilisateur(UtilisateurDto dto, HttpServletRequest request) {
		...
		if (utilisateur == null) {
			...
			oldValue = "Nom: " + utilisateur.getNom() + ", Prénom: " + utilisateur.getPrenom() + ", Opérateur: "
					+ oldOperateur + " et Profils: " + oldRoles;
		}
		...
		if (user.getOperateur() != null) {
			utilisateur.setOperateur(user.getOperateur());
		}
		...
		String newValue = "Nom: " + utilisateur.getNom() + ", Prénom: " + utilisateur.getPrenom() + ", Opérateur: "
				+ newOperateur + " et Profils: " + newRoles;

		Utilisateur userSave = utilisateurRepository.save(utilisateur);
	...
	public List<ProfilDto> getAllRoles() {
		List<Profil> profils = profilRepository.getAllProfilExterne();
	...
	public List<UtilisateurDto> getUtilisateursDBByParam(UserCriteresDto dto) {
		...
		if (currentUser != null) {
			...
			List<Object> objects = utilisateurRepository.getListUsersExterneByParams(dto.getLogin(), nom, prenom,
			...
			if (listUsers != null && CollectionUtils.isNotEmpty(listUsers)) {
				...
				String operation = "Récupération de la base de données la liste des utilisateurs de tailles: "
						+ dtos.size() + " et par critères de recherche: [LOGIN]: " + dto.getLogin() + ", [NOM]: "
						+ dto.getNom() + ", [Prénom]: " + dto.getPrenom();
	...
	public Utilisateur save(Utilisateur utilisateur) {
		String login = SecurityContextHolder.getContext().getAuthentication().getName();
		Utilisateur user = utilisateurRepository.findByLogin(login);
		Utilisateur saveUser = null;
		if (user != null) {
			utilisateur.setOperateur(user.getOperateur());
			if (utilisateur.getOperateur() != null) {
				saveUser = utilisateurRepository.save(utilisateur);
			}
